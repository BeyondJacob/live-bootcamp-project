...
mod tests {
    ...
    async fn test_add_token() {
        ...
        let is_banned = store.is_banned(&token).await.unwrap();
        assert!(is_banned);
    }

    #[tokio::test]
    async fn test_is_banned_returns_false_for_unknown_token() {
    ...
    async fn test_add_multiple_tokens() {
        ...
        assert!(store.is_banned(&token1).await.unwrap());
        assert!(store.is_banned(&token2).await.unwrap());
        assert!(store.is_banned(&token3).await.unwrap());
        assert!(!store.is_banned("unknown").await.unwrap());
    }
